...
function _asset_preview_text(preview) {
...
function _choose_asset_duplicate_mode(preview) {
    ...
    return swal({
        ...
        text: _asset_preview_text(preview) + '\n\nChoose how to handle existing assets in the case.',
        icon: 'warning',
        buttons: {
            cancel: true,
            skip: {
...
function _confirm_asset_upload(preview) {
    return swal({
...
function upload_assets() {
    ...
    reader.onload = function (e) {
        ...
        post_request_api('/case/assets/upload/preview', JSON.stringify(previewPayload), true)
            .done((previewResponse) => {
                ...
                _confirm_asset_upload(preview).then((shouldContinue) => {
                    if (!shouldContinue) {
                        return;
                    }

                    _choose_asset_duplicate_mode(preview).then((duplicateMode) => {
                        if (!duplicateMode) {
                            return;
                        }
                        ...
                        post_request_api('/case/assets/upload', JSON.stringify(uploadPayload), true)
                            .done((data) => {
                                if (data.status == 'success') {
                                    reload_assets();
